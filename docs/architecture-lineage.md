...
| Runtime | Simulates fill-level OOP, ranks plans by rules and fit score, optionally reranks with the hybrid model, and surfaces counselor-ready outputs and research summaries. | Streamlit, CLI, evaluation reports |
...
| `evaluate_hybrid_reranker()` | training CSV and saved model artifacts | compares rules-only, heuristic, linear, tree, and ablation systems |
