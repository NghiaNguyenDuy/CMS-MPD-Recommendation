...
> [!note]
> This revised draft incorporates the updated `cms-mpd` knowledge base and a repository review completed on 2026-04-08.

## Abstract

**Background:** Medicare Part D plan choice remains difficult because beneficiary welfare depends on medication-specific coverage, utilization-management restrictions, pharmacy-channel access, low-income subsidy status, and contract-year policy rules rather than premium alone. The updated `cms-mpd` knowledge base also shows that January 1, 2024 and January 1, 2025 policy changes improved affordability while increasing the importance of date-aware interpretation, counseling, and explanation.  
...
**Materials and Methods:** The manuscript synthesizes reviewed source notes in the `cms-mpd` vault together with direct inspection of the local repository, including the DuckDB data pipeline, recommendation engine, modeling workflow, and checked-in evaluation artifacts for the `2025-Q3` snapshot. Particular attention is given to related work, data logic and transformation, runtime recommendation workflow, and the constrained role of machine learning in reranking.  
...
Choosing a Medicare Part D plan is a case-specific decision problem, not a standard consumer price-comparison task. A beneficiary's best option depends on whether each medication is covered, how it is tiered, whether prior authorization, step therapy, or quantity limits apply, whether the beneficiary relies on retail or mail fulfillment, and whether low-income subsidy status changes actual out-of-pocket liability. The updated note set in [[Source - Plan Selection and Decision Support Evidence]] shows that many beneficiaries fail to choose the least-cost or near-least-cost plan without guidance. The counseling notes in [[Source - SHIP Counseling and Beneficiary Navigation]] show that real plan comparison is therefore a workflow problem as much as a data problem.

This task became more sensitive after the staged redesign of Part D. The evidence synthesized in [[Source - Part D Reform and IRA Timeline]] shows that the catastrophic 5 percent beneficiary coinsurance ended on January 1, 2024, and that the redesigned $2,000 annual out-of-pocket cap took effect on January 1, 2025. The same knowledge base links those reforms to the Medicare Prescription Payment Plan, expanded full-subsidy treatment under Extra Help, continuing insulin protections, and the first negotiated prices that took effect on January 1, 2026. These changes improved affordability, but they also made year-specific interpretation more important because older evidence, older beneficiary materials, and older pricing intuitions no longer map cleanly to the post-redesign environment.

The present project, `CMS-MPD-Recommendation`, addresses this environment as a counselor-oriented decision-support platform. The system is not intended to replace formal claims adjudication or to claim state-of-the-art status in generic recommendation research. Instead, it combines a DuckDB medallion pipeline, beneficiary-specific fill-level simulation, explanation-first plan comparison, and an optional constrained reranking layer. The updated `cms-mpd` vault is especially useful here because it ties together plan-selection evidence, SHIP workflow, communication barriers, pricing distortions, insurer incentives, and current Part D operational guidance in a single research frame.
...
The literature most directly matched to this study concerns beneficiary plan choice and decision support. The reviewed evidence in [[Source - Plan Selection and Decision Support Evidence]] consistently shows that Medicare beneficiaries often do not enroll in the most cost-effective Part D plan. Administrative-data studies in that note report average avoidable spending on the order of several hundred dollars per year, while the CHOICE decision-aid work shows that guided tools can improve alignment with expert recommendations even when decisional-conflict measures do not change sharply. This body of work matters because it frames the problem as a recurring failure of unaided plan selection rather than as a theoretical optimization exercise.

That same evidence also clarifies what a useful plan-comparison tool must do. Narrowing choices alone is insufficient if beneficiaries cannot interpret annual cost exposure, drug restrictions, or plan-detail pages. The updated knowledge base therefore supports a design in which recommendation quality depends on cost simulation, clear tradeoff presentation, and explanation categories that match how people actually review plans.

### 2.2 Counseling, navigation, and communication infrastructure

The second relevant body of work concerns counseling and communication. [[Source - SHIP Counseling and Beneficiary Navigation]] treats SHIP as the main neutral, local infrastructure for Medicare navigation, with counselors translating prescriptions, LIS questions, enrollment timing, and misinformation into actionable plan choices. This is not a peripheral observation. It implies that a practical Part D system should be counselor-compatible and explanation-first, not just score-driven.

The communication literature reaches the same conclusion from another angle. [[Source - Medicare Communication and Beneficiary Readability]] shows that official Medicare materials can be technically accurate yet still exceed the reading level of many beneficiaries, especially in Part D sections involving formularies, enrollment, appeals, and low-income assistance. Together, these notes suggest that explanation is not an optional user-interface layer. It is part of the task definition. A system that produces rankings without interpretable coverage, access, and cost logic would be poorly matched to the communication burden documented in the literature.

### 2.3 Policy redesign, market change, and sponsor operations

The third strand of related work concerns the redesign-era policy environment. [[Source - Part D Reform and IRA Timeline]] and [[Source - Extra Help and LIS Operations]] show that Part D after January 1, 2025 cannot be analyzed with the same assumptions used for older four-phase benefit discussions. Full subsidy treatment expanded beginning January 1, 2024 for broader low-income groups, the annual out-of-pocket cap changed the structure of beneficiary liability in 2025, and the payment-smoothing option changed the timing of payments even when it did not change the total owed.

The market and operational notes broaden this picture. [[Source - Part D Market Snapshots and Enrollment Trends]] shows that enrollment keeps growing while stand-alone PDP menus shrink, sponsor concentration remains high, and benchmark-plan pressure continues to matter for LIS beneficiaries. [[Source - Part D Operations, Enrollment, and Bidding Guidance]] shows that bids, formularies, pricing files, enrollment windows, and reporting obligations are tightly structured CMS processes rather than loosely defined plan behaviors. These materials matter because they anchor the study in the actual operating environment of Part D, not only in beneficiary-facing abstractions.

### 2.4 Formulary strategy, pricing distortions, and insurer incentives

Another relevant literature addresses why plan design can look inconsistent or confusing from the beneficiary side. [[Source - Drug Pricing and Formulary Distortions]] shows that list price, generic status, negotiated cost, and beneficiary out-of-pocket cost do not map neatly to one another. The same note synthesizes work on formulary exclusions, utilization-management growth, preferred-brand cost sharing, selected-drug coverage, and price-signal distortions in which some brand drugs can create lower beneficiary cost than generics. This literature strongly supports fill-level plan simulation rather than premium-first or generic-first heuristics.

[[Source - Part D Insurer Incentives and Market Design]] adds a program-design explanation for this behavior. The NBER work synthesized there shows that Part D formularies and pricing strategies reflect not only clinical coverage choices but also the interaction of risk adjustment, subsidy design, integration incentives, and selection incentives. [[Source - MedPAC Part D Status Reports and Oversight]] complements that perspective by showing how reinsurance, LIS financing, and high-cost drug classes shape the program-level context in which plans operate. Together, these sources support a study design that treats plan behavior as policy-shaped and sponsor-incentive-sensitive rather than as simple retail pricing.

### 2.5 Health recommender systems and the task mismatch problem

Broader health recommender research is relevant for positioning, but it is not the direct comparator. Existing scoping reviews of health recommender systems show a heterogeneous field in which knowledge-based and hybrid methods remain common when safety, interpretability, and controllability matter more than raw predictive lift. The present study fits that pattern. It does not recommend medications from electronic health records or optimize repeated consumer interactions. It ranks insurance plans for a known medication regimen under policy, geography, subsidy, and access constraints.
...
The objective of this manuscript is to present `CMS-MPD-Recommendation` clearly as an integrated Medicare Part D decision-support study and to assess what the project contributes relative to the updated `cms-mpd` knowledge base and the current codebase.
...
6. It creates a single research environment in which knowledge-base synthesis, runtime recommendation, synthetic or PDE-compatible scenario generation, model fitting, and evaluation all use the same serving layer.
...
### 4.1 Knowledge-base and repository review

This manuscript is grounded in two linked evidence sources. The first is the reviewed `cms-mpd` vault, especially [[Source - Plan Selection and Decision Support Evidence]], [[Source - SHIP Counseling and Beneficiary Navigation]], [[Source - Part D Reform and IRA Timeline]], [[Source - Extra Help and LIS Operations]], [[Source - Insulin Affordability in Part D]], [[Source - Drug Pricing and Formulary Distortions]], [[Source - Part D Operations, Enrollment, and Bidding Guidance]], [[Source - Part D Market Snapshots and Enrollment Trends]], [[Source - MedPAC Part D Status Reports and Oversight]], [[Source - Part D Insurer Incentives and Market Design]], and [[Source - Medicare Communication and Beneficiary Readability]].
...
| `cms-mpd` knowledge base | reviewed source notes and topic notes | policy framing, related work, and interpretation |
...
### Reviewed `cms-mpd` knowledge-base notes

1. [[Source - Plan Selection and Decision Support Evidence]]
2. [[Source - SHIP Counseling and Beneficiary Navigation]]
3. [[Source - Medicare Communication and Beneficiary Readability]]
4. [[Source - Part D Reform and IRA Timeline]]
5. [[Source - Extra Help and LIS Operations]]
6. [[Source - Insulin Affordability in Part D]]
7. [[Source - Drug Pricing and Formulary Distortions]]
8. [[Source - Part D Operations, Enrollment, and Bidding Guidance]]
9. [[Source - Part D Market Snapshots and Enrollment Trends]]
10. [[Source - MedPAC Part D Status Reports and Oversight]]
11. [[Source - Part D Insurer Incentives and Market Design]]

### Local project artifacts

12. [README](../../../README.md)
13. [architecture-lineage.md](../../../docs/architecture-lineage.md)
14. [technical-data-flow-modeling.md](../../../docs/technical-data-flow-modeling.md)
15. [study-research-flow-data-logic-algorithm.md](../../../docs/study-research-flow-data-logic-algorithm.md)
16. [hybrid_reranker_dataset.metadata.json](../../../data/training/2025-Q3/full/hybrid_reranker_dataset.metadata.json)
17. [hybrid_reranker_evaluation_tree.json](../../../data/training/2025-Q3/full/hybrid_reranker_evaluation_tree.json)

### Broader positioning literature already cited in the project draft

18. Cai Y, Yu F, Kumar M, Gladney R, Mostafa J. Health recommender systems development, usage, and evaluation from 2010 to 2022: a scoping review. *International Journal of Environmental Research and Public Health*. 2022;19(22):15115.
19. Ananthakrishnan A, Milne-Ives M, Cong C, Meinert E. The evaluation of health recommender systems: a scoping review. *International Journal of Medical Informatics*. 2025;195:105697.
